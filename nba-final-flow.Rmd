...
I've opted for 15% coverage of plays (or cases) which contain the activity pairs as most would be sparce and beyond evaluating. 

In line with the taxonomy, this event can be categorised as:
\- A **scope** which is segmented and globally occurring, that is continuously occurring. Having more comprehensive game data on off the ball movement may have changed this.
\- A **shape** which is combined was an order close to FIFO-variable speed (FIFO stands for fly-in fly-out, thanks to @eca for the assist on that meaning)
\- A **workload** which is continuous in some activity pairs and sparse in others
